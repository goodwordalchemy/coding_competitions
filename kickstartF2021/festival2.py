...
def solution(D, K, rides):
    ...
    idx_to_rank = {i: rank+1 for rank, (_, i) in enumerate(hs)}
    ...
    for _, type_, h, i in line_sweep:
        if type_ == 1:
            enabled.update(idx_to_rank[i], 1)
            scores.update(idx_to_rank[i], h)
            idx = enabled.idx_at_or_below(K)+1
            if idx == 0:
                idx = len(rides) + 1
            
            result = max(result, scores.query(1, idx))
        else:
            enabled.update(idx_to_rank[i], 0)
            scores.update(idx_to_rank[i], 0)
